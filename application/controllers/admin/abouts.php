...
class Abouts extends Admin_controller {
  ...
  public function __construct () {
    ...
    $this->feature_name_1 = '簡介';
    $this->object_name_1  = 'About';

    $this->feature_name_2 = '簡介照片';
    $this->object_name_2  = 'AboutPicture';
  }
  ...
  public function detail ($id, $submenu_num = 0) {
    ...
    if (verifyNumber ($id) && verifyObject ($object = $object_name::find ('one', array ('conditions' => array ('id = ?', $id))))) {
      ...
      $sql = array ('order'  => 'sort DESC, id DESC',
                    ...
                    'conditions' => array ('about_id = ?', $object->id));
  ...
  public function create_picture ($id) {
    ...
    if (verifyNumber ($id) && verifyObject ($object = $object_name::find ('one', array ('conditions' => array ('id = ?', $id))))) {
      ...
      if (verifyUploadFormat ($picture) && verifyString ($title)) {
        $sql = array (
          'title'       => $title,
          'about_id'    => $object->id,
          'file_name'   => '',
          'sort'        => (verifyObject ($last_object = $object_name::find ('one', array ('order' => 'sort DESC', 'conditions' => array ('about_id = ?', $object->id)))) ? $last_object->sort : 0) + 1
        );
  ...
  public function update_picture ($id) {
    ...
    if (verifyNumber ($id) && verifyObject ($object = $object_name::find ('one', array ('conditions' => array ('id = ?', $id))))) {
      ...
      if (verifyUpdateUpload ($picture, $object->file_name) && verifyString ($title)) {
        ...
        $this->message ('成功', '修改成功!', base_url (array ('admin', $this->get_class (), 'detail', $object->about_id)));
      } else { 
        $data['object'] = $object;
        $data['update_url'] = base_url (array ('admin', $this->get_class (), $this->get_method (), $object->id));
        $data['back_url'] = base_url (array ('admin', $this->get_class (), 'detail', $object->about_id));
  ...
  public function delete_picture ($id) {
    ...
    if (verifyNumber ($id) && verifyObject ($object = $object_name::find ('one', array ('conditions' => array ('id = ?', $id))))) {
      if ($this->is_ajax (false)) {
        $object->delete ();
        delay_request ('delay_jobs', 'delete_cache_about');
        $this->output_json (array ('status' => true, 'title' => '成功', 'message' => '刪除成功!', 'action' => 'function(){window.location.assign ("' . base_url (array ('admin', $this->get_class (), 'detail', $object->about_id)) . '");}'));
      } else { 

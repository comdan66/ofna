...
if (!function_exists ('verifyArrayFormat')) {
  function verifyArrayFormat ($obj = array (), $items = array ()) {
    return verifyArray ($items) && verifyArray ($obj) && verifyArray (array_filter (array_map (function ($item) use ($obj) { return array_key_exists ($item, $obj) ? true : null;}, $items))) ? true : false;
  }
}
